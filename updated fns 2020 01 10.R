...
# also added a function to process results without 4 change-point results
# added extra 4cp functions for IDM results

#### Weibull functions ####
weib.nll <- function(par, dta){
  #gamma <- par[1]
  #theta <- par[2]
  #ll <- log(theta) * sum(dta$censor) + (gamma - 1) * sum(dta$censor * log(dta$time)) -
  #  (theta / gamma) * sum(dta$time ^ gamma)
...
weib.4cp.LRT<-function(tau, gamma = 2, dta) {
  G <- sum(dta$censor)
  T <- (1/gamma) * sum(dta$time^gamma)
  G1 <- sum(dta$time < tau[1])
  G2 <- sum((tau[1] <= dta$time) * (dta$time < tau[2]))
...
weib.4cp.nll.theta <- function(theta, tau, gamma, dta) {
  theta1<-theta[1]
  theta2<-theta[2]
  theta3<-theta[3]
  theta4<-theta[4]
  ...
  ll<-(g-1)*sum(dta$censor*log(dta$time))+
    ...
    log(theta3)*sum((tau[2]<=dta$time)*(dta$time<tau[3]))+
    log(theta4)*sum((tau[3]<=dta$time)*(dta$time<tau[4]))+
    log(theta5)*sum((dta$time>=tau[4])*dta$censor)-
    (theta1/g)*sum((dta$time^g)*(dta$time<tau[1]) +
                     (tau[1]^g)*(dta$time>=tau[1]))-
    (theta2/g)*sum((dta$time^g-tau[1]^g)*(dta$time>=tau[1])*(dta$time<tau[2])+
                     (tau[2]^g-tau[1]^g)*(dta$time>=tau[2]))-
    (theta3/g)*sum((dta$time^g-tau[2]^g)*(dta$time>=tau[2])*(dta$time<tau[3])+
                     (tau[3]^g-tau[2]^g)*(dta$time>=tau[3]))-
    (theta4/g)*sum((dta$time^g-tau[3]^g)*(dta$time>=tau[3])*(dta$time<tau[4])+
                     (tau[4]^g-tau[3]^g)*(dta$time>=tau[4]))-
    (theta5/g)*sum((dta$time^g-tau[4]^g)*(dta$time>=tau[4]))
  return(-ll)
}
...
# Full nll in terms of theta
exp.4cp.nll.theta <- function(theta, tau, dta){ 
  theta1 <- theta[1]
  theta2 <- theta[2]
  theta3 <- theta[3]
  theta4 <- theta[4]
  theta5 <- theta[5]
  ll<-sum(dta$censor*log(dta$time))+
    log(theta1)*sum((dta$time < tau[1]))+
    log(theta2)*sum((tau[1]<=dta$time)*(dta$time<tau[2]))+
    log(theta3)*sum((tau[2]<=dta$time)*(dta$time<tau[3]))+
    log(theta4)*sum((tau[3]<=dta$time)*(dta$time<tau[4]))+
    ...
    (theta2)*sum((dta$time-tau[1])*(dta$time>=tau[1])*(dta$time<tau[2])+
                   (tau[2]-tau[1])*(dta$time>=tau[2]))-
    (theta3)*sum((dta$time-tau[2])*(dta$time>=tau[2])*(dta$time<tau[3])+
                   (tau[3]-tau[2])*(dta$time>=tau[3]))-
    (theta4)*sum((dta$time-tau[3])*(dta$time>=tau[3])*(dta$time<tau[4])+
                   (tau[4]-tau[3])*(dta$time>=tau[4]))-
    (theta5)*sum((dta$time-tau[4])*(dta$time>=tau[4]))
  return(-ll)
}
...
results.3cp <- function(resdta, cp, nsim=10000) {
  if(cp == 0) {
    nocp.BICres<-data.frame(nocp.BIC.1cp = resdta$BIC1cp,
                            nocp.BIC.2cp = resdta$BIC2cp, 
                            nocp.BIC.3cp = resdta$BIC3cp)
    ...
    ## Type I error (run for nocp results)
    nocp.alpha<-sum(nocp.LRT.test1cp<probs1cp.alpha, nocp.LRT.test2cp<probs2cp.alpha, 
                    nocp.LRT.test3cp<probs3cp.alpha)/nsim
    nocp.spending<-sum(nocp.LRT.test1cp<probs1cp.spending, nocp.LRT.test2cp<probs2cp.spending, 
                       nocp.LRT.test3cp<probs3cp.spending)/nsim
  ...
  if(cp == 1) {
    ...
    ##alpha
    #power
    cp1.power.alpha<-sum(cp1.LRT.test1cp<probs1cp.alpha, cp1.LRT.test2cp<probs2cp.alpha, 
                         cp1.LRT.test3cp<probs3cp.alpha)/nsim 
    #sensitivity
    cp1.sensitivity.alpha<-sum(cp1.LRT.test1cp<probs1cp.alpha)/nsim 
    ##alpha spending
    #power
    cp1.power.spending<-sum(cp1.LRT.test1cp<probs1cp.spending, cp1.LRT.test2cp<probs2cp.spending, 
                            cp1.LRT.test3cp<probs3cp.spending)/nsim 
    #sensitivity
    cp1.sensitivity.spending<-sum(cp1.LRT.test1cp<probs1cp.spending)/nsim 
  ...
  if(cp == 2) {
    ...
    #power
    cp2.power.alpha<-sum(cp2.LRT.test1cp<probs1cp.alpha, cp2.LRT.test2cp<probs2cp.alpha, 
                         cp2.LRT.test3cp<probs3cp.alpha)/nsim 
    ...
    #power
    cp2.power.spending<-sum(cp2.LRT.test1cp<probs1cp.spending, cp2.LRT.test2cp<probs2cp.spending, 
                            cp2.LRT.test3cp<probs3cp.spending)/nsim 
  ...
  if(cp == 3) {
    ...
    #power
    cp3.power.alpha<-sum(cp3.LRT.test1cp<probs1cp.alpha, cp3.LRT.test2cp<probs2cp.alpha, 
                         cp3.LRT.test3cp<probs3cp.alpha)/nsim 
    ...
    #alpha spending
    cp3.power.spending<-sum(cp3.LRT.test1cp<probs1cp.spending, cp3.LRT.test2cp<probs2cp.spending, 
                            cp3.LRT.test3cp<probs3cp.spending)/nsim 
...
save(weib.1cp.nll, weib.1cp.LRT, weib.1cp.BIC,
     ...
     exp.4cp.nll.theta,
     exp_invcdf,
     is.error, test_cp, results, results.3cp,
     file = "C:/Users/Camille/Documents/D word/4 Writing the thing/simulation 2019 11 20/dwordfns20200112.RData")
